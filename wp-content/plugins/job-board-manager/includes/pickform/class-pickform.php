...
class class_pickform{
	...
	public function sanitizations($input_value, $input_type){
		...
		elseif($input_type=='textarea'){
			...
			$input_value = wp_kses( $input_value, $filter );
...
*/
			...
			$field_html = apply_filters('job_bm_filter_input_field_html', $field_html);
	...
	public function field_text_multi($field_data){
            ...
            if(!empty($field_values)){
                if(is_array($field_values)){
                    ...
                    foreach($field_values as $key=>$value){
						...
						$html.= '<input class="remove-field" type="button" value="'.__('Remove', pickform_textdomain).'" />';						
						$html.= '</div>';
            ...
            $html.= '</div>';
            $html.= '<input class="add-field" option-id="'.$field_id.'" type="button" value="'.__('Add more', pickform_textdomain).'" />';			
            $html.= '</div>';			
	...
	public function field_file($field_data){
			...
			if(!empty($input_values)){
				foreach($input_values as $file_id){
					...
					if(!empty($attachment_url)){
						$html.= '<div class=item attach_id="'.$file_id.'"><img src="'.wp_get_attachment_url($file_id).'" /><span attach_id="'.$file_id.'" class=delete>Delete</span><input  type=hidden name='.$field_id.'[] value="'.$file_id.'" /></div>';
						}
				...
				$html.= '<input type="hidden" value="0" name="'.$field_id.'[]">';
				} 
			...
			//$html.= '<input id="'.$field_id.'" type="hidden" placeholder="" name="'.$field_id.'[]" value="0" /> ';
			//$html.= '<input type="hidden" value="0" name="'.$field_id.'[]">';
						
			$html.= '<p>'.__("Drop files here", pickform_textdomain).'</p>
						<p class="drag-drop-buttons"><input id="plupload-browse-'.$field_id.'" type="button" value="'.__("Select Files", pickform_textdomain).'" class="button" /></p>
					  </div>
					 </div>
				  </div>
  ...
  $plupload_init = array(
    ...
    'filters'             => array(array('title' => __('Allowed Files', pickform_textdomain), 'extensions' => 'gif,png,jpg,jpeg')),
    'multipart'           => true,
  ...
  $plupload_init = apply_filters('plupload_init', $plupload_init);
	...
	public function field_wp_editor($field_data){
			...
			ob_start();
			wp_editor( stripslashes($field_values), $field_id, $settings = array('textarea_name'=>$field_id, 'media_buttons'=>false,'wpautop'=>true,'teeny'=>true,'editor_height'=>'200px', ) );				
