...
class Meow_MediaFileRenamer {
	...
	function init() {
		...
		$this->mfrh_admin = new MFRH_Admin();
		...
		$auto_rename = get_option( 'mfrh_auto_rename' );
		if ( $auto_rename != 'none' ) {
	...
	function admin_menu() {
		$auto_rename = get_option( 'mfrh_auto_rename' );
		if ( $auto_rename != 'none' ) {
			add_media_page( 'Media File Renamer', __( 'Media File Renamer', 'media-file-renamer' ), 'manage_options', 'rename_media_files', array( $this, 'rename_media_files' ) );
		}
	}
	...
	function check_attachment( $id, &$output = array() ) {
		...
		if ( file_exists( $path_parts['dirname'] . "/" . $desired_filename ) ) {
			$is_numbered = get_option( 'mfrh_numbered_files' );
			if ( $this->mfrh_admin->is_pro() && $is_numbered ) {
	...
	function attachment_fields( $post ) {
		...
		echo '<input type="text" ' . ( $is_manual && $this->mfrh_admin->is_pro() ? '' : 'readonly' ) . ' class="widefat" name="mfrh_new_filename" value="' . $basename. '" />';
		if ( !$is_manual ) {
			echo '<p class="description">You need to enable <b>Manual Rename</b> in the plugin settings.</p>';
		}
		else if ( !$this->mfrh_admin->is_pro() ) {
			echo '<p class="description">This feature is for <a target="_blank" href="http://meowapps.com/media-file-renamer/">Pro users</a> only.</p>';
		}
		else {
	...
	function log_sql( $data, $antidata ) {
		if ( !get_option( 'mfrh_logsql' ) || !$this->mfrh_admin->is_pro() )
			return;
	...
	function new_filename( $media, $title, $forceFilename = null ) {
		...
		if ( $force )
			...
			$utf8_filename = get_option( 'mfrh_utf8_filename' ) && $this->mfrh_admin->is_pro();
	...
	function rename_media( $post, $attachment, $disableMediaLibraryMode = false, $forceFilename = null ) {
		...
		if ( $manual && !$this->mfrh_admin->is_pro() ) {
			return $post;
		}
		...
		$force_rename = get_option( 'mfrh_force_rename' ) && $this->mfrh_admin->is_pro();
	...
	function init_actions() {
		...
		if ( get_option( "mfrh_sync_alt" ) && $this->mfrh_admin->is_pro() )
