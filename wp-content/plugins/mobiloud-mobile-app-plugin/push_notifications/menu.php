...
function ml_push_notification_manual_send_callback() {
	if ( isset( $_POST['ml_push_notification_msg'] ) ) {
		...
		if ( strlen( $_POST['ml_push_notification_data_id'] ) > 0 ) {
			...
			} else if ( strpos( $_POST['ml_push_notification_data_id'], 'url' ) !== false ) {
				$url = $_POST['ml_push_notification_url'];
			} else {
	...
	<table class="wp-list-table widefat fixed posts">
		...
		<?php if ( count( $notifications ) ): ?>
			<?php foreach ( $notifications as $notification ): ?>
				...
				if (!empty($notification->url)) $attach[] = 'url:"' . $notification->url . '"';
				?>
	...
	<div class="ml_send_notification_box">
					...
					<p>
						...
						<label>
							<input id="ml_push_notification_os_all" type="radio" name='ml_push_notification_os'
								   value="all" checked/> All (<?php echo $total_count; ?> total devices)
						</label><br/>
						<label>
							<input id="ml_push_notification_android" type="radio" name='ml_push_notification_os'
								   value="android"/> Android only (<?php echo $android_count; ?> devices)
						</label><br/>
						<label>
							<input id="ml_push_notification_ios" type="radio" name='ml_push_notification_os'
								   value="ios"/> iOS only (<?php echo $ios_count; ?> devices)
						</label>
					</p>
		...
		<p class="submit">
			<input type="submit" onclick="return false;" class='button button-primary button-large'
				   id="ml_push_notification_manual_send_submit" value="<?php _e( 'Send' ); ?>"/>
		</p>
