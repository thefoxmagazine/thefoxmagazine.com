<div class='ml-preview <?php echo strlen( get_option( 'ml_preview_os' ) ) ? get_option( 'ml_preview_os' ) : 'ios'; ?>'
     onclick='jQuery("#ml_reminder").show();'>
	<div class='ml-preview-body'>
		...
		<div
			class='ml-preview-article-list <?php echo get_option( 'ml_article_list_view_type', 'extended' ) == 'extended' ? 'ml-view-extended' : 'ml-view-compact'; ?>'>
			<div class='scroller'>
				<?php
				...
				if ( count( $posts ) > 0 ) {
					...
					foreach ( $posts as $post ) {
						...
						<div class='ml-preview-article'>
							...
							<?php endif; ?>
							<div
								class='ml-preview-article-body' <?php echo ! $imageUrl ? 'style="width:100%;"' : ''; ?>>
								...
								<span
									class='ml-article-date'><?php echo( get_option( 'ml_datetype', 'prettydate' ) == 'prettydate' ? Mobiloud_App_Preview::how_long_ago( strtotime( $post->post_date ) ) : date_i18n( get_option( 'ml_dateformat', 'F j, Y' ), strtotime( $post->post_date ), get_option( 'gmt_offset' ) ) ); ?></span>
							</div>
							<div class="clear"></div>
						</div>
						<?php
				...
				<div id='ml-page-placeholder'>
					<h3>Your page will display here</h3>
					<small>Use the Live Preview to see the final result.</small>
				</div>
			</div>
		</div>
	</div>
