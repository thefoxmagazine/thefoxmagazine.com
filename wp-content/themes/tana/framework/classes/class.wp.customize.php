...
global $themeton_less_variables;
$themeton_less_variables = array();
if( !function_exists('getLessValue') ){
    function getLessValue($name){
        global $themeton_less_variables;
        if( empty($themeton_less_variables) ){
            $themeton_less_variables = is_user_logged_in() ? TTLess::get_less_variables() : array();
        }

        if( isset($themeton_less_variables[$name]) ){
            $val = $themeton_less_variables[$name];
            $val = str_replace("'", "", $val);
            $val = str_replace('"', '', $val);
            return $val;
        }
...
global $themeton_customizer_options;
function get_customizer_options(){
    global $themeton_customizer_options;
    if( !empty($themeton_customizer_options) ){
        return $themeton_customizer_options;
    }
    
    $themeton_customizer_options = tt_customizer_options();
    return $themeton_customizer_options;
}
...
function get_customizer_controls(){
    ...
    $controls = array();
    foreach ($themeton_customizer_options as $panel) {
...
class TT_Theme_Customizer{
    ...
    public function generate_setting($wp_customize, $control, $section_id=false){
        $transport_mode = ThemetonStd::get_mod('transport_mode');
        $transport_mode = !empty($transport_mode) ? $transport_mode : 'refresh';
        $wp_customize->add_setting($control['id'], array(
            'default' => isset($control['default']) ? $control['default'] : '',
            'type' => 'theme_mod',
            'transport' => $transport_mode, //refresh | postMessage
            'capability' => 'edit_theme_options',
